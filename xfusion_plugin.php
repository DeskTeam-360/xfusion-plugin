...
/**
 * [leadership_checkbox a=1 ac=0 c=1 l=1 e=0]
 * a=green Alignment, ac=orange Accountability, c=blue Communication,
 * l=red Leadership, e=purple Execution. 1 = checked, 0 = empty outline.
 */
function xfusion_leadership_checkbox_shortcode($atts = []): string
{
    $atts = shortcode_atts([
        ...
        'l'  => '0',
        'e'  => '0',
    ], $atts, 'leadership_checkbox');

    $items = [
        'a'  => ['Alignment',      '#4caf50'],
        'ac' => ['Accountability', '#f5a623'],
        'c'  => ['Communication',  '#3b82f6'],
        'l'  => ['Leadership',     '#e53935'],
        'e'  => ['Execution',      '#9b59b6'],
    ];

    $row = 'display:inline-flex;align-items:center;gap:6px;vertical-align:middle;';
    $box = 'width:22px;height:22px;border-radius:4px;box-sizing:border-box;flex-shrink:0;display:inline-flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;line-height:1;color:#fff;';

    $html = '<span class="xfusion-leadership-checkboxes" style="' . $row . '">';

    foreach ($items as $key => $item) {
        $on = in_array(strtolower(trim((string) $atts[$key])), ['1', 'true', 'yes', 'on'], true);

        if ($on) {
            $style = $box . 'background:' . $item[1] . ';';
            $mark = '&#10003;';
        } else {
            $style = $box . 'background:transparent;border:2px solid #c8d4e0;';
            $mark = '';
        }

        $html .= '<span title="' . esc_attr($item[0]) . '" style="' . esc_attr($style) . '">' . $mark . '</span>';
    }
