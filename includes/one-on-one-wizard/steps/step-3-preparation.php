...
preparation: function () {
    ...
    var scaleIcons = {
        alignment_clarity: iconBase + 'Arrow-in-Target-Icon.svg',
        workload_sustainability: iconBase + 'Engine-Gauge-Icon.svg',
        confidence_priorities: iconBase + 'Checkmark-Shield-Icon.svg',
        priority_alignment: iconBase + 'Dot-and-List-Icon.svg',
        observed_progress: iconBase + 'Progressing-Business-Chart-Icon.svg',
        support_effectiveness: iconBase + 'Handle-With-Care-Icon.svg',
    };
    function scaleQ(role, fieldKey, label, desc, min, max, selected) {
        ...
        var iconSrc = scaleIcons[fieldKey] || '';
        var iconHtml = iconSrc
            ? '<img class="xfw-scale-q-icon" src="' + iconSrc + '" alt="" width="40" height="40">'
            : '';
        return '<div class="xfw-scale-q" data-field="' + fieldKey + '" data-type="scale">' +
            '<div class="xfw-scale-q-head">' + iconHtml +
            '<div class="xfw-scale-q-text"><label>' + label + '</label><div class="q-desc">' + desc + '</div></div></div>' +
    ...
    return '<h2 class="xfw-section-title">Step 3. Shared Preparation™</h2>' +
        ...
        '<div class="xfw-section-label" style="margin-top:1rem;font-weight:800;color:var(--navy);font-size:24px;">Open Reflections</div>' +
