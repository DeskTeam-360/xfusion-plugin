<?php
/**
 * ARR Step 6 — AI Strategic Renewal Synthesis™ Laravel bridge.
 *
 * @package XFusion
 */
...
add_action('wp_ajax_xfarr_synthesis_generate', function (): void {
    ...
    xfarr_picker_send(xfarr_picker_api_request('POST', "/{$arrId}/synthesis/generate", [], [
        'user_id' => get_current_user_id(),
    ]));
});

add_action('wp_ajax_xfarr_synthesis_load', function (): void {
    check_ajax_referer('xfarr_wizard', 'nonce');
    if (! is_user_logged_in()) {
        wp_send_json_error(['message' => 'Unauthorized.'], 401);
    }
    ...
    xfarr_picker_send(xfarr_picker_api_request('GET', "/{$arrId}/synthesis", [
        'user_id' => get_current_user_id(),
    ]));
});

function xfarr_wizard_synthesis_service_js(): string
{
    return <<<'JS'
window.xfarrGenerateSynthesis = function () {
    if (!window.XFARR_WIZARD || !window.XFARR_WIZARD.arrId) {
        return Promise.reject(new Error('No ARR selected.'));
    }
    var payload = new URLSearchParams();
    payload.set('action', 'xfarr_synthesis_generate');
...
window.xfarrLoadSynthesis = function () {
    if (!window.XFARR_WIZARD || !window.XFARR_WIZARD.arrId) {
        return Promise.resolve(null);
    }
    var params = new URLSearchParams();
    params.set('action', 'xfarr_synthesis_load');
