 ...
 *
 * Renders just the grid of tools for one wp_course_groups category, each
 * tagged with which of the 5 COR Organizational Capabilities it addresses
 * (wp_fusion_tool_scoring_group_tags). No title/description wrapper - the
 * page itself (e.g. an Elementor section) supplies the heading and copy;
 * this shortcode only outputs the data grid.
 *
 * Usage:
 *   [fusion_tool_library group="Leadership Development"]   (match by title)
 *   [fusion_tool_library group_id="4"]                      (match by id)
 *
 * @package XFusion
 */
...
function xfusion_tool_library_shortcode($atts = []): string
{
    ...
    if ($groupId > 0) {
        $courseGroup = $wpdb->get_row($wpdb->prepare(
